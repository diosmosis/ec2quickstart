...
CONNECT_SSH_FAILURE_THRESHOLD = 10
...
class StartAmiOp(OperationBase):
    ...
    def get_ip_address(self, progress):
        ...
        handle = urllib2.urlopen('http://whatismyip.com/automation/n09230945.asp')
        self.ip = handle.read()
        handle.close()
...
class TerminateOp(OperationBase):
    ...
    def terminate_instances(self, progress):
        reserv = self.conn.get_all_instances()
        for x in reserv:
            for y in x.instances:
                progress.some_progress_made('Terminating \'' + y.id + '\'...')
                y.terminate()
